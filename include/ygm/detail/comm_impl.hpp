...
public:
  ...
  void async_final_flush(int index) {
    if (index != m_comm_local_rank) {
      ...
      free_buffer(buffer);
    } else { // For terminal messages, we swap the buffer and add it to the
             // arrival queue
      if (final_send_buffers[index]->size() == 0)
        return;
      auto buffer = allocate_buffer();
...
private:
  ...
  // void receive_queue_push_back(std::shared_ptr<std::vector<char>> b, int
  // from) {
  ...
  //   }
  //   if (current_size > 16) {
  //     std::this_thread::sleep_for(std::chrono::microseconds(current_size -
  //     16));
  //   }
  // }
